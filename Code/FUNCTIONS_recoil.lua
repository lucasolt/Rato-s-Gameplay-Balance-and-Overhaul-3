...
function GetRecoilOther(weapon, attacker, action, persistent)
    ...
    if not attacker.placeholder then
        ...
        if action and
            ...
            if weapon.BurstSelective or weapon:HasComponent("Reduce_recoil_burst_delta") then
...
function Rat_GetRecoilBaseMod(attacker, action, weapon, num_shots, persistent)
    ...
    ---- Marksmanship -- medido, da o mesmo numero nas tres posturas. NAO e `weapon_recoil_mod`: o
    ---- calibre no seu piso, tracer, AP e os deltas de rajada tambem sao da arma e vivem dentro de
...
---------------------------------------------------------------------------------------------------
---- Postura, perks, Marksmanship e Forca-vs-calibre. Devolve os DOIS lados da MESMA passada --
---- GetRecoilOther e GetCaliberStrRecoil ja calculam ambos, e chama-las de novo so para separar
---- duplicaria o metaText:
----   control        -- so o subproduto que o ATIRADOR manda, em BASE 100: 100 = nao cancela nada,
----                     77 = cancelou 23%. E o que o modelo angular vira chance de segurar o cano.
...
function Rat_GetRecoilControl(attacker, action, weapon, num_shots, metaText, dual, weapon2,
    ...
    situation_mod = situation_mod * str_caliber_mod
