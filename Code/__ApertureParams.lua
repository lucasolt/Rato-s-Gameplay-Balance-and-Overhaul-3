...
function GBO_ApplyApertureCTHMode(mode)
    if mode then
        RAT_ApertureCTHMode = mode
    end
    local m = RAT_ApertureCTHMode
    const.Combat.Aperture.Enabled =
        (m == "aCTH" or m == "aCTH Lite")
    const.Combat.Aperture.SimulateShots =
        (m == "aCTH")
    for _, u in ipairs(
                    g_Units or
                        empty_table) do
...
local function apply_mode_and_field()
    GBO_ApplyApertureCTHMode(
        CurrentModOptions and
            CurrentModOptions.ApertureCTH)
    if Rat_ReapplyApertureComponents and
        g_Units and #g_Units > 0 then
